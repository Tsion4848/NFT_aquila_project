...
const ImageStorage = multer.diskStorage({
  ...
  const uploadImageFile = multer({ storage: ImageStorage,  }).single("imageFilePath")
app.post("/uploadImage", async(req, res)=>{
    uploadImageFile(req, res, err=>{
        if(err){
            console.log(err);
            return res.status(404).json({success:false, message:"Image Not Uploaded"})
        }
...
app.post('/upload', (req, res) => {
    ...
    const {photoname, description, photographer, floorprice, auctiondate, imagePath} = req.body
    const image = imagePath
    db.query('INSERT INTO upload (photoname, description, photographer, floorprice, auctiondate, image) VALUES (?, ?, ?, ?, ?, ?)',
    [photoname, description, photographer, floorprice, auctiondate, image], (err, result) => {
        if (err) {
